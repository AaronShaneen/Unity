...
namespace MatchingGameTemplate
{
    ...
    public class MGTGameController : MonoBehaviour
    {
        ...
        [Tooltip("A list of all the pairs in the game. These pairs are for sound+text. If an MGTPairsTextSound component is attached to this gamecontroller, it will override the default pairs array.")]
        [HideInInspector]
        public PairTextSound[] pairsTextSound = null;
        ...
        [Tooltip("A list of all the pairs in the game. These pairs are for image+text. If an MGTPairsImageText component is attached to this gamecontroller, it will override the default pairs array.")]
        [HideInInspector]
        public PairImageText[] pairsImageText = null;
        ...
        [Tooltip("The menu that appears if we lose the game")]
        ...
        void Start()
        {
            ...
            //     UpdateLevelTextText();
            // }
            if (pairsImageImage.Length > 0)
            {
                // Set the pairs type
                pairsType = "ImageImage"; //start with an image image match
            ...
            if (pairsImageSound.Length > 0)
            {
                if (randomizePairs == true) pairsImageSound = ShuffleImageSoundPairs(pairsImageSound);
            }

             if (pairsImageText.Length > 0)
            { 
                if (randomizePairs == true) pairsImageText = ShuffleImageTextPairs(pairsImageText);
            }

            if (pairsTextSound.Length > 0)
            {
               
                if (randomizePairs == true) pairsTextSound = ShuffleTextSoundPairs(pairsTextSound);
            }     
        }
        ...
        public void SelectObject(Transform selectedObject)
        {
            if (firstOfPair == true) // Selecting the first object of a pair
            {
                ...
                // If we are playing a Sound-image or text sound game mode, make the image buttons interactable again, so we can select one of them
                ...
                if (pairsType == "TextSound") foreach (RectTransform imageButton in firstGroup) imageButton.GetComponent<Button>().interactable = true;
            }
            else // Selecting the second object of a pair
            {
                ...
                // If we selected two objects, check if they match. In Image - Text mode we compare the image of the object to the name of the other object. In Text - Text mode we compare the pairs and see if they have the same firstText and secondText values
                else if (pairsType == "ImageText"   && (firstObject.Find("Image").GetComponent<Image>().sprite.name == selectedObject.Find("Image").GetComponent<Image>().sprite.name 
                        ...
                        || pairsType == "TextSound" && (firstObject.Find("Image").GetComponent<Image>().sprite.name == selectedObject.Find("Image").GetComponent<Image>().sprite.name 
                                                    || selectedObject.Find("Image").GetComponent<Image>().sprite.name == firstObject.Find("Image").GetComponent<Image>().sprite.name) 
                        )
                {
                ...
                if (pairsType == "ImageSound" || pairsType == "TextSound")
                {
        ...
        /// <param name="pairs">A list of text strings</param>
         ...
         PairImageText[] ShuffleImageTextPairs(PairImageText[] pairs)
        {
            // Go through all the text strings and shuffle them
            for (index = 0; index < pairs.Length; index++)
            {
                // Hold the text string in a temporary variable
                PairImageText tempPair = pairs[index];
        ...
        PairImageSound[] ShuffleImageSoundPairs(PairImageSound[] pairs)
        {
         ...
         PairTextSound[] ShuffleTextSoundPairs(PairTextSound[] pairs)
        {
            // Go through all the text strings and shuffle them
            for (index = 0; index < pairs.Length; index++)
            {
                // Hold the text string in a temporary variable
                PairTextSound tempPair = pairs[index];
		...
		IEnumerator LevelUp(float delay)
        {
            ...
            if (pairsType == "ImageImage") pairsType = "ImageSound";
            else if (pairsType == "ImageSound") pairsType = "ImageText";
            else if (pairsType == "ImageText") pairsType = "TextSound";
            else if (pairsType == "TextSound") 
            {
                victory = true;
            ...
            if ( !victory && (currentPair < pairs.Length || currentPair < pairsTextText.Length || currentPair < pairsImageSound.Length || currentPair < pairsImageImage.Length))
            {
                currentLevel++;
                ...
                if (pairsType == "ImageImage") UpdateLevel();    
                else if (pairsType == "ImageSound") UpdateLevelImageSound();
                else if (pairsType == "ImageText") UpdateLevelImageText();
                else if (pairsType == "TextSound") UpdateLevelTextSound();
        ...
        /// Updates the level, showing the next set of pairs ( Used for Image - Text matching )
        /// </summary>
        void UpdateLevelImageText()
        {
            // Display the current level we are on
            if (levelText) levelText.text = levelNamePrefix + " " + (currentLevel).ToString();

            // If we have a default button assigned and we have a list of pairs, duplicate the button and display all the pairs in the grid
            if (defaultButton && pairsImageText.Length > 0)
            {
                // Deactivate the default button as we don't need it anymore
                defaultButton.gameObject.SetActive(false);
                ...
                // Create the shop buttons by duplicating the default one
                for (index = currentPair; index < pairsImageText.Length; index++)
                {
                    // Create a new button
                    RectTransform newButton = Instantiate(defaultButton) as RectTransform;
                    ...
                    if (firstOfPair == true)
                    {
                        ...
                        newButton.Find("Image").GetComponent<Image>().enabled = true; //Turn on image
                        newButton.Find("SoundIcon").GetComponent<Image>().enabled = false; //turn off sound icon
                        newButton.Find("Image").GetComponent<Image>().sprite = pairsImageText[index].image;
                    ...
                    else
                    {
                        ...
                        newButton.Find("Image").GetComponent<Image>().enabled = false;
                        newButton.Find("SoundIcon").GetComponent<Image>().enabled = false; //turn off sound icon
                        newButton.Find("Image").GetComponent<Image>().sprite = pairsImageText[index].image; // The image of the sprite is disabled however this is used when comparing the two objects when selected.
                        newButton.Find("Text").GetComponent<Text>().text = pairsImageText[index].text;
                        newButton.Find("Text2").GetComponent<Text>().text = pairsImageText[index].text;
        ...
        /// <summary>
        /// Updates the level, showing the next set of pairs ( Used for Text - Sound matching )
        /// </summary>
        void UpdateLevelTextSound()
        {
            // Display the current level we are on
            if (levelText) levelText.text = levelNamePrefix + " " + (currentLevel).ToString();

            // If we have a default button assigned and we have a list of pairs, duplicate the button and display all the pairs in the grid
            if (defaultButton && pairsTextSound.Length > 0)
            {
                // Deactivate the default button as we don't need it anymore
                defaultButton.gameObject.SetActive(false);
                ...
                // Create the shop buttons by duplicating the default one
                for (index = currentPair; index < pairsTextSound.Length; index++)
                {
                    // Create a new button
                    RectTransform newButton = Instantiate(defaultButton) as RectTransform;
                    ...
                    if (firstOfPair == true)
                    {
                        ...
                        newButton.Find("Text").GetComponent<Text>().enabled = true;
                        newButton.Find("Image").GetComponent<Image>().enabled = false;
                        newButton.Find("Image").GetComponent<Image>().sprite = pairsTextSound[index].image;
                        newButton.Find("Text").GetComponent<Text>().text = pairsTextSound[index].text;
                        newButton.Find("Text2").GetComponent<Text>().text = pairsTextSound[index].text;
                    ...
                    else
                    {
                        ...
                        newButton.Find("Image").GetComponent<Image>().enabled = false;
                        newButton.Find("Image").GetComponent<Image>().sprite = pairsTextSound[index].image;
                        newButton.GetComponent<MGTPlaySound>().sound = pairsTextSound[index].sound;
